...
const LoginSection = () => {
    ...
    const formik = useFormik({
        ...
        validationSchema: yup.object({
            email: yup.string().email("Your Email is not valid! Provide a valid email").required("Email is required"),
        ...
        onSubmit: async (values, { resetForm }) => {
            try {
                ...
                if (response.data && response.data.access_token) {
                    // Success: Store token in cookies and navigate
                    Cookies.set('token', response.data.access_token, { expires: 10, secure: true, sameSite: 'Strict' }); // Store token in cookies
                    navigate('/account', { state: { userData: response.data } });
                    resetForm();
                                ...
                                <div className="col-lg-6">
                                    <div className="loginRegister-content">
                                        <form onSubmit={formik.handleSubmit} method="POST">
                                            <h3 className="loginRegister__title text-poppins">Login to Mannetha Infra</h3>
                                            <p className="loginRegister__desc mb-4 font-18">Login to Mannetha Infra and seamlessly manage your real estate operations with confidence. Your gateway to streamlined project management and business growth.</p>
